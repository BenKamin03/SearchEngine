...
public class HtmlCleaner {
	...
	public static String stripEntities(String html) {
		// Regular expression to match HTML entities with decimal or hexadecimal syntax
		String entityPattern = "&(?:#([0-9]+)|#x([0-9a-fA-F]+)|([a-zA-Z0-9]+));";

		// Compile the pattern for efficiency
		Pattern pattern = Pattern.compile(entityPattern);

		// Matcher for finding entities in the HTML
		Matcher matcher = pattern.matcher(html);
		...
		while (matcher.find()) {
			String replacement = StringEscapeUtils.unescapeHtml4(matcher.group());
